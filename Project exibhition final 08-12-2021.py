...
def transpose(canvas):
    ...
    if canvas.data.image!=None:
        imageData = np.asarray(canvas.data.image)
        newimg=cv2.flip(imageData, -1)
...
def noise(canvas):
    ...
    canvas.data.drawOn=False
    if canvas.data.image!= None:
        #print(canvas.data.image)
        pic = np.asarray(canvas.data.image)# pil to numpy array
        #print(pic)
        #plt.imshow(pic)
        con_pic = cv2.cvtColor(pic, cv2.COLOR_BGR2RGB)
        newimg = cv2.fastNlMeansDenoisingColored(con_pic,None,10,10,7,21)
        #plt.imshow(newimg)
        newimg = Image.fromarray(newimg) #nummpy to pil
...
def newImage(canvas):
    ...
    try: filetype=imghdr.what(imageName)
        ...
        tkinter.messagebox.messagebox.showinfo(title="Image File",\
...
def desktopBk(canvas):
    if canvas.data.image!=None:
        loc = str(canvas.data.imageLocation)
        #print(loc)
        SPI_SETDESKWALLPAPER = 20  
        ctypes.windll.user32.SystemParametersInfoW(SPI_SETDESKWALLPAPER, 0, str(loc), 0)
...
def buttonsInit(root, canvas):
    ...
    rotateButton=Button(toolKitFrame, text="Transpose",\
                        background=backgroundColour, \
                        width=buttonWidth,height=buttonHeight, \
    ...
    noiseButton=Button(toolKitFrame, text="Noise",\
                       background=backgroundColour ,width=buttonWidth,\
                       height=buttonHeight, command=lambda: noise(canvas))
    noiseButton.grid(row=11,column=0)
